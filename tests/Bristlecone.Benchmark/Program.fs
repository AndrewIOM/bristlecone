...
let runReplicated nTimes work =
    [| 1 .. nTimes |]
    |> Array.Parallel.map(fun _ -> measureTime (fun () -> work ()))
...
let summariseRuns modelName (optimName: string) trueMinima minVal minima (results: ((Bristlecone.EstimationEngine.Solution<float> list * float array) * int) array) =
...
module TimeSeriesTests =
    ...
    let summarise modelName optimName (results: (Result<Bristlecone.Test.TestResult,string> * int) seq) =    
        let successes = 
            ...
            |> List.map(fun r -> match fst r with | Ok x  -> (x, snd r) | Error _ -> invalidOp "")
        {
            ModelName = modelName
            OptimMethod = optimName
            ...
            DistanceFromLikelihoodMedian = successes |> List.map fst |> List.map(fun r -> abs <| r.RealLikelihood - r.EstimatedLikelihood) |> median
...
let timeModels = [
    "predator-prey (with gaussian noise)",
        ...
        Bristlecone.Test.addStartValues [
            "lynx", 1.0
            "hare", 1.0 ]
    "predator-prey", 
...
module Output =
    ...
    let markdown optimTable timeSeriesTable = sprintf "
Bristlecone Benchmarks
===\n
## Optimisation Methods\n\n%s\n
## Full Model Systems\n\n### Time-series models\n\n%s" optimTable timeSeriesTable

    let markdownTable columnNames rowData =
        sprintf "| %s |\n| %s |\n%s" (columnNames |> String.concat " | ")
            (columnNames |> Seq.map(fun _ -> "---") |> String.concat " | ")
            (rowData |> Seq.map(fun d -> sprintf "| %s |" (d |> String.concat " | " )) |> String.concat "\n")

    let fileName = "../../benchmarks.md"

    let paramEstimatesToCell (p:EstimatedP list) =
        p |> Seq.map(fun p -> sprintf "best %.3f / median %.3f (±%.3f)" p.Best p.Median p.StDev )
        |> String.concat "<br>"
    ...
    let summariseParameters (p:list<list<Bristlecone.Test.ParameterTestResult>>) =
        ...
        |> List.map(fun (g,v) ->
            let distancesFromReal = (v |> Seq.map(fun x -> abs(x.EstimatedValue - x.RealValue)))
            sprintf "[%s] %.3f best / median %.3f (±%.3f)" g
                (distancesFromReal |> Seq.min) (distancesFromReal |> median)
                (distancesFromReal |> Statistics.Statistics.StandardDeviation))
        |> String.concat "<br>"

    let pointsToCell points =
        points |> List.map(fun l -> l |> List.map threeDp |> String.concat " / ")
        |> String.concat "<br>"
    ...
    then 
        ...
        let optimTable =
            Output.markdownTable
                [ "Model name"; "Optimisation method"; "n runs";
                    "Target minimum"; "Best estimated minimum"; "Median estimated minimum";
                    "Sum distance from true parameters"; "Parameter estimates (individual)"; 
                    "True solutions"; "Milliseconds taken (median)" ]
                (optimResults |> Seq.map(fun o ->
                    [ o.ModelName; o.OptimMethod; o.Runs.ToString(); 
                      Output.threeDp o.MinimumReal; sprintf "%s (%s -> min)" (Output.threeDp o.MinimumEstimatedBest) (Output.threeDp o.DistanceFromMinimumBest); Output.threeDp o.MinimumEstimatedMedian;
                      Output.threeDp o.ParameterSpaceDistanceMedian; Output.paramEstimatesToCell o.ParameterEstimates;
                      Output.pointsToCell o.MinimumRealPoints; o.MillisecondsMedian.ToString() ]))
        ...
        let timeTable =
            Output.markdownTable
                [ "Model name"; "Optimisation method"; "success %"; "n runs"; 
                                "Distance from minimum likelihood (median)";
                                "Distance from true parameter values"; "Milliseconds taken (median)" ]
                (timeResults |> Seq.map(fun o ->
                    [ o.ModelName; o.OptimMethod; Output.threeDp (float o.SuccessfulRuns / float o.Runs * 100.0) + "%"; o.Runs.ToString();
                      Output.threeDp o.DistanceFromLikelihoodMedian;
                      Output.summariseParameters o.Parameters; o.MillisecondsMedian.ToString() ]
                    ))
