...
type CustomGen() =
    static member Floats() : Arbitrary<float list> =
        genMultiList 2 1000 |> Arb.fromGen
...
let config = { FsCheckConfig.defaultConfig with arbitrary = [typeof<CustomGen>] }
...
let timeSeries =
    testList "Time series" [
        ...
        testPropertyWithConfig config "Resolution is always the same as creation-time" <| fun date (data:float list) resolution ->
            let ts = data |> TimeSeries.fromSeq date resolution
            Expect.equal ts.Resolution (Resolution.Fixed resolution)
                "Resolution was transformed during processing"
        
        testPropertyWithConfig config "Start time is always the same as creation-time" <| fun date (data:float list) resolution ->
            let ts = data |> TimeSeries.fromSeq date resolution
            ts.StartDate.Deconstruct() |> snd = date

        testPropertyWithConfig config "Create from observations fails without 2+ observations" <| fun (data:TimeSeries.Observation<float> list) ->
            if data |> List.length < 2
            then Expect.throws (fun () -> TimeSeries.fromObservations data |> ignore) 
        ...
        testPropertyWithConfig config "'map' transforms all values" <| fun (observations:TimeSeries.Observation<float> list) f ->
            let ts : TimeSeries<float> = observations |> TimeSeries.fromObservations
            let original = ts.Values
            Expect.sequenceEqual (ts |> TimeSeries.map(fun (d,_) -> f d)).Values (original |> Seq.map f)
                "map behaved differently between Seq.map and TimeSeries.map"

        testList "Bounding functions" [
            testPropertyWithConfig config "Trimming start removes all time points before specified date" <| fun startDate (observations:TimeSeries.Observation<float> list) ->
                let trimmed = observations |> TimeSeries.fromObservations |> TimeSeries.trimStart startDate
                let x = observations |> Seq.filter(fun (_,d) -> d > startDate) |> Seq.map fst
                if x |> Seq.length < 2
                then Expect.isNone trimmed "There were less than two observations left"
                else 
                    Expect.isSome trimmed "There were more than one observation"
                    Expect.equal (trimmed.Value.Values |> Seq.length) (x |> Seq.length) "Count differed between seq and TimeSeries"
                    Expect.sequenceEqual (trimmed.Value.Values |> Seq.sort) (x |> Seq.sort) "Values differed between seq and TimeSeries"

            testPropertyWithConfig config "Trimming end removes all time points after specified date" <| fun date (obs:TimeSeries.Observation<float> list) ->
            ...
            testPropertyWithConfig config "Start date cannot be before given start date when bounding" <| fun startDate endDate (obs:TimeSeries.Observation<float> list) ->
                let bounded = obs |> TimeSeries.fromObservations |> TimeSeries.bound startDate endDate
                match bounded with
                | Some ts -> Expect.isGreaterThanOrEqual (ts.StartDate.Deconstruct() |> snd) startDate "Start date was earlier than start cut-off"
                | None -> ()

            testPropertyWithConfig config "Bounding removes time points before and after dates" <| fun startDate endDate (obs:TimeSeries.Observation<float> list) ->
                let trimmed = obs |> TimeSeries.fromObservations |> TimeSeries.bound startDate endDate
                let x = obs |> Seq.filter(fun (_,d) -> d >= startDate && d <= endDate) |> Seq.map fst
                if x |> Seq.length < 2
                then Expect.isNone trimmed "There were less than two observations left"
                else 
                    Expect.isSome trimmed "There were more than one observation"
                    Expect.equal (trimmed.Value.Values |> Seq.length) (x |> Seq.length) "Count differed between seq and TimeSeries functions"
                    Expect.sequenceEqual (trimmed.Value.Values |> Seq.sort) (x |> Seq.sort) "Values differed between seq and TimeSeries"
