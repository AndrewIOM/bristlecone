...
type ShortCodeGen() =
    ...
    static member EquationList =
        genStrings 1 10 |> tupleGen<ModelExpression> |> Arb.fromGen
        ...
        gen {
            ...
            return
                ...
                |> List.map(fun (c,b1,b2) -> 
                    c, Parameter.create noConstraints b1.Get b2.Get |> Option.get)
...
let modelExpressions =
    testList "Model expression - compute" [
        ...
        testProperty "Getting parameter value fails when parameter not present" <| fun code x t pool e ->
        ...
        testPropertyWithConfig config "Retrieves environment when present" <| fun identifier (value:NormalFloat) x t pool ->
...
let modelBuilder =
    testList "Model builder" [
        ...
        testProperty "Cannot add an equation with a blank identifier" <| fun name eq ->
            if String.IsNullOrEmpty name
            then Expect.throws 
                    (fun () -> Model.empty |> Model.addEquation name eq |> ignore)
                    "Allowed a blank identifier"

        testPropertyWithConfig config "Does not compile when no equations are specified" <| fun (eqs:(string * ModelExpression) seq) ->
            let mb = eqs |> Seq.fold (fun mb (n,eq) -> mb |> Model.addEquation n eq) Model.empty
            let fn () = mb |> Model.compile
            if eqs |> Seq.length <> 1
            then Expect.throws (fun () -> fn() |> ignore) "Did not throw when no equations specified"
        ...
        testPropertyWithConfig config "Compiles whether measures are present or not" <| fun likelihood eq1 measures ->
            let model =
                Model.empty 
