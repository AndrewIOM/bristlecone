...
module Solver =
    ...
    let variableExternalStep logTo timeHandling timeSteps (initialPoint:CodedMap<float>) =
        ...
        | Continuous i -> fun eqs ->
            let results =
                timeSteps
                |> Seq.pairwise
                |> Seq.scan(fun (state:CodedMap<float>) (t0:float,t1:float) ->
                    i logTo t0 t1 (t1-t0) state Map.empty eqs
                    |> Map.map(fun k v -> v.[0])) initialPoint
            eqs 
            |> Map.toSeq 
            |> Seq.map(fun (key,_) -> key, results |> Seq.map(fun r -> r.[key]) |> Seq.toArray)
            |> Map.ofSeq

    let fixedResolutionSolver fRes stepType (dynamicSeries:TimeFrame.TimeFrame<'a>) (environment:TimeFrame.TimeFrame<'a> option) engine t0 =
        let timeline,forcings =
            ...
            | Some f -> 
                ...
                | Resolution.Fixed efRes ->
                    engine.LogTo <| DebugEvent "Solving along the timeline of supplied environmental forcing data."
                    let timeline = 
                        (dynamicSeries.Series |> Seq.head).Value 
                        |> TimeIndex.indexSeries dynamicSeries.StartDate efRes 
                        |> Seq.map fst
                    let envIndex = f.Series |> Map.map(fun k v -> TimeIndex.TimeIndex(dynamicSeries.StartDate, efRes, TimeIndex.IndexMode.Interpolate Statistics.Interpolate.lower, v)) //TimeIndex.IndexMode.Interpolate Statistics.Interpolate.bilinear, v))
                    (timeline, envIndex)
            | None -> 
                engine.LogTo <| DebugEvent "No environmental forcing data was supplied. Solving using time points of observations."
                let timeline = 
                    (dynamicSeries.Series |> Seq.head).Value 
        ...
        let externalSteps = timeline |> Seq.pairwise |> Seq.map(fun (a,b) -> b - a) |> Seq.distinct |> Seq.toList
        if externalSteps.Length <> 1 then failwithf "Encountered uneven timesteps: %A" externalSteps
        let solve = fixedStep engine.LogTo engine.TimeHandling (startIndex - externalSteps.Head) endIndex t0 forcings
        fun ode ->
            match stepType with
            | Internal -> solve ode
            | External ->
    ...
    let solver stepType (dynamicSeries:TimeFrame.TimeFrame<'a>) (environment:TimeFrame.TimeFrame<'a> option) engine t0 : Solver<'a> =
        match dynamicSeries.Resolution with
        | Resolution.Fixed fRes ->
            engine.LogTo <| DebugEvent (sprintf "Observations occur on a fixed temporal resolution: %A." fRes)
        ...
        | Resolution.Variable ->
            ...
            | None -> 
                ...
                engine.LogTo <| DebugEvent "Solving in time units of years. This is not configurable at present."
                let timeline = (dynamicSeries.Series |> Seq.head).Value.TimeSteps |> Seq.map(fun t -> t.Days)
