...
module Likelihood =
    ...
    let bivariateGaussian' (p:ParameterPool) obsx obsy expx expy = 
        ...
        // Mike 2004:
        // let likelihood =  
        //     1. / (2. * System.Math.PI * sqrt (1. - rho ** 2.)) 
        //     * exp ( (1. / (2. * (1. - rho ** 2.))) * z)

        // Mike / Lizzy C code
        let likelihood =
            1. / (2. * System.Math.PI * sigmax * sigmay * sqrt(1. - (rho*rho)))
            * exp (-z / (2. * (1. - (rho * rho))))

        // printfn "L = %f (-logL = %f) (sigmax = %f; sigmay = %f; rho = %f; diffx %f; diffy %f; z1 = %f; z2 = %f; z3 = %f)" likelihood (-log(likelihood)) sigmax sigmay rho diffx diffy zta1 zta2 zta3
        -log(likelihood)
