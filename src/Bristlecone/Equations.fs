...
module Likelihood =
    ...
    let gaussian (key:ModelSystem.LikelihoodRequirement) : Likelihood<parameter> =
        ...
        |> fun f -> { Evaluate = f; RequiredCodes = [ key ] }
    ...
    /// </summary>
    let bivariateGaussian (key1: ModelSystem.LikelihoodRequirement) (key2:ModelSystem.LikelihoodRequirement) : Likelihood<'u> =
        fun (paramAccessor: ParameterValueAccessor) data ->
            ...
            |> Typed.sumVector
        |> fun f -> { Evaluate = f; RequiredCodes = [ key1; key2 ] }
