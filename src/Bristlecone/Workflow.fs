    ...
    type OrchestrationAgent(writeOut, maxSimultaneous, retainResults) =
        ...
        let agent =
            MailboxProcessor.Start(fun inbox ->
                let rec messageLoop inProgress =
                    async {
                        ...
                        let preProcess =
                            ...
                            | StartWorkPackage work ->
                                ...
                                print writeOut
                                <| sprintf "Work parcel added to queue (%i items are waiting)" queue.Count

                                inProgress
                            | Finished r ->
                                print writeOut
                                <| sprintf "Work parcel has finished (%i items are waiting)" queue.Count

                                if retainResults then
                                    results.Enqueue r
                            ...
                            | WorkFailed e ->
                                print writeOut
                                <| sprintf "There was an issue completing one work item: %s" e.Message
                        ...
                        let postProcess =
                            if preProcess < maxSimultaneous && queue.Count > 0 then
                        ...
                        print writeOut
                        <| sprintf "Running %i work parcels in parallel (limit = %i)" inProgress maxSimultaneous
